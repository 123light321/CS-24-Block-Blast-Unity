    ...
    void Start()
    {
        ...
        if (bl_GameManager.Instance.m_GameMode == GameMode.PickUp)
        {
            ...
            if (found)
            {
                ...
                {
                    GameObject obj = Instantiate(m_Root.ItemPrefab[0], InstancePosition, Quaternion.identity) as GameObject;
                    GameObject obj1 = Instantiate(m_Root.ItemPrefab[1], InstancePosition, Quaternion.identity) as GameObject;
                    obj.transform.parent = transform;
                    obj1.transform.parent = transform;
                    obj.transform.position = new Vector3(transform.position.x, transform.position.y, transform.position.z + 10);
                }
            }
        }
    }
