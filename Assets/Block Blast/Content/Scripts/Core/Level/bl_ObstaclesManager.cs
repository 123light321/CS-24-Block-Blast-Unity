...
public class bl_ObstaclesManager : Singleton<bl_ObstaclesManager>
{
    ...
    public float maxWaitBetweenObstacles = 4f;
    public float maxWaitBetweenObstaclesInfinity = 4f;
    ...
    void Start()
    {
        ...
        if (UsePoolMethod)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                GameObject pol = Instantiate(ItemPrefab[0]) as GameObject;
                GameObject enemy = Instantiate(ItemPrefab[1]) as GameObject;
                pol.transform.parent = ItemParent;
                enemy.transform.parent = ItemParent;
                enemy.transform.position = new Vector3(transform.position.x, transform.position.y, transform.position.z + 10);
                PoolItemArray.Add(pol);
                PoolItemArray.Add(enemy);
                pol.SetActive(false);
    ...
    IEnumerator SpawnSecunce()
    {
        ...
        while (true)
        {
            bool isTutorial = PlayerPrefs.GetInt(KeyMasters.Tutorial, 0) < 3;
            if (isTutorial) { 
            }

            else if (UsePoolMethod) {
